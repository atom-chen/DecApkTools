...
local AngelSanction = class("AngelSanction", SkillBase)
function AngelSanction:ctor(pActor, id, pData)
  AngelSanction.super.ctor(self, pActor, id, pData)
  if id == 1023 then
...
function AngelSanction:Update(dt)
  AngelSanction.super.Update(self, dt)
end
function AngelSanction:Execute(endCallback)
...
function AngelSanction:IsTriggered()
  local supCondition = AngelSanction.super.IsTriggered(self)
...
function AngelSanction:FindTarget()
...
function AngelSanction:IsTarget(pActor)
  if self.m_pActor == pActor then
    return false
  end
  if not pActor:IsCanAttacked() or pActor:IsHex() or pActor:IsZombie() then
    return false
  end
